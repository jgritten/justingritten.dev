...
const ROADMAP_PHASES = [
  ...
  },
  {
    ...
    items: [
      ...
      'Done: Clerk for /saas + API JWT validation + GET /api/v1/me; post-sign-in hub route before dashboard',
      ...
      'Optional later: first-party username/password login (API + client) if not using Clerk for that path',
      ...
      'Email verification (optional); more OAuth providers via Clerk as needed',
    ],
  },
  {
    id: 'phase-2',
    ...
    completed: false,
    items: [
      'Product direction: Permit & Compliance Management demo on the shared SaaS template (see docs/roadmap.md)',
      'Template stop-gap before domain depth: permissions end-to-end, file storage, minimal audit pattern',
      'Client (tenant) entity, memberships, invitations, workspace preferences API (ADR 0011)',
      'Post-sign-in hub: invites, client list, Create Client → wizard; default client + optional skip-hub',
      'Current client context; multi-client and switch client in shell',
      'Permissions: define, assign to roles, enforce in API, hide/disable in UI (Applicant/Reviewer-ready)',
      'Files: platform/global library (operator uploads) + tenant-private storage (strict isolation; optional entity links)',
      'Minimal audit events; client-visible audit log and timelines build on the same pattern',
      'Shared data per client; client admin; client default theme; support impersonation and data manipulation',
      'Soft delete; document generation (template + microserver, rate limit); document packages; version/revision history',
      'File validation; email notifications',
      'Permit MVP slice after template gate: applications, workflow, attachments via tenant-private files',
    ],
  },
  {
    id: 'phase-3',
  ...
  },
  {
    ...
    completed: false,
    items: [
      'Permit & Compliance platform: extend workflow (revisions, notifications, generated letters) per roadmap notes',
