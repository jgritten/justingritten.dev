...
- **SaaS tenancy (authenticated):** `GET/PUT /api/v1/Tenancy/workspace|preferences`, `POST /api/v1/Tenancy/clients`, invitation accept/decline, **`GET/PATCH /api/v1/Tenancy/clients/members`**, **`DELETE /api/v1/Tenancy/clients/invitations/{id}`** (revoke pending invite; with **`X-Tenant-Client-Id`**) — Clerk JWT; see [ADR 0011](decisions/0011-multi-tenant-clients-and-workspace-hub.md), [ADR 0013](decisions/0013-x-tenant-client-id-and-members-api.md), [ADR 0014](decisions/0014-revoke-pending-tenant-invitation.md). The SPA hub is `/saas/post-sign-in`. **`GET workspace`** ensures a pending invite to the seeded **Northwinds Demo** client when the JWT includes a normalized email and the user is not already a member and has no prior invite row for that tenant+email ([ADR 0012](decisions/0012-northwinds-demo-tenant-and-auto-invite.md)).
